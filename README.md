...
Default is **1 hour of math per day**, shown as four 15-minute blocks. Change it behind the **gear** in the masthead, under **Daily math target** — the grid resizes itself, so 240 gives you sixteen blocks in four labelled rows. Past entries are stored as a count of 15-minute units, so changing the target never rewrites history.

### Still showing the old target after a deploy?

Settings are saved per browser, and a saved value always beats the file's default. If the app was ever used on that browser with a different target, that saved number is what you will see — a new deploy cannot change it.

Open the **gear** and read the build line at the bottom. It shows the build ID, the file's default, and what this device is set to. If those last two disagree, click **Reset settings to default** — it restores the target and session dates and keeps every logged day and passed level.
